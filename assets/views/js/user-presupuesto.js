...
let selectedItemIndex, selectedPresupuestoIndex, selectedItemIndexPost;
...
var myModalItemEditPost = document.getElementById('modalItemEditPost');
var modalItemEditPost = bootstrap.Modal.getOrCreateInstance(myModalItemEditPost);
...
$(document).ready(function (e) {
    ...
    table_items_edit.on('select', function (e, dt, type, indexes) {
        if (type === 'row') {
            selectedItemIndex = indexes[0]
            $('#btnEditItemPost').removeClass('d-none');
        }
    });

    table_items_edit.on('deselect', function (e, dt, type, indexes) {
        if (type === 'row') {
            $('#btnEditItemPost').addClass('d-none');
        }
    });
...
function reserFormItemPost() {

    $('#formItemPresupuestoEditPost').removeClass('was-validated');
    $('#inputDescripcionEditPost').val()
    $('#inputDienteEditPost').val()
    $('#inputObservacionEditPost').val()
    $('#inputValorEditPost').val()
    $('#inputDesarrolloEditPost').val()
    $('#inputEstadoEditPost').val()
    $('#inputFechaEditPost').val()
}

$('#btnEditItemPost').click(function (e) {

    let selectedData = table_items_edit.rows({selected: true}).data()[0];
    idItemPresupuesto = selectedData.iditem_presupuesto;

    $.ajax({
        url: baseUrl + '/AdminPresupuesto/getDataItemPresupuesto',
        type: 'POST',
        data: {
            idItemPresupuesto: idItemPresupuesto,
        },
        success: function (result) {
            ...
            selectedItemIndexPost = idItemPresupuesto;
            $('#inputDescripcionEditPost').val(data.descripcion)
            $('#inputDienteEditPost').val(data.diente)
            $('#inputObservacionEditPost').val(data.observacion)
            $('#inputValorEditPost').val(data.valor)
            $('#inputDesarrolloEditPost').val(data.desarrollo)
            $('#inputEstadoEditPost').val(data.estado_pago)
            $('#inputFechaEditPost').val(data.fecha_pago)

            modalItemEditPost.show();
...
$('#btnMore').click(function (e) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#pres_number').text(idPresupuesto);
            showLoader(false);
...
$('#btnGuardarItemPost').click(function (e) {

    let desarrollo = $('#inputDesarrolloEditPost').val()
    let estado = $('#inputEstadoEditPost').val()
    let fecha = $('#inputFechaEditPost').val()

    if (desarrollo && estado && fecha) {
        $.ajax({
            url: baseUrl + '/AdminPresupuesto/editItemPresupuesto',
            type: 'POST',
            data: {
                idItemPresupuesto: selectedItemIndexPost,
                desarrollo: desarrollo,
                estado: estado,
                fecha: fecha
            },
            success: function (result) {
                ...
                if (result == 'true') {
                    sendToast('success', 'Actualización', 'Informacion actualizada')
                } else {
                    sendToast('success', 'Error', 'Error al actualizar, contacte al administrador')
                }

                modalItemEditPost.hide();
                updateTablaItemPost();

            }
        });
    } else {
        sendToast('secondary', 'Faltan Datos', 'Complete los datos Obligatorios')
    }
})

function updateTablaItemPost() {
    let selectedData = table.rows({selected: true}).data()[0];
    idPresupuesto = selectedData.idpresupuesto;

    $.ajax({
        url: baseUrl + '/AdminPresupuesto/getDataItemsPresupuesto',
        type: 'POST',
        data: {
            idPresupuesto: idPresupuesto
        },
        success: function (result) {
            res = JSON.parse(result);
            table_items_edit.clear().draw();
            table_items_edit.rows.add(res).draw();
        }
    });
}
